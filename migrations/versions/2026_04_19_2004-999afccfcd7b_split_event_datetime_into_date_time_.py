...
def upgrade() -> None:
    ...
    op.execute(
        'UPDATE events SET date = start::date, time_start = start::time, time_end = "end"::time'
    )
...
def downgrade() -> None:
    ...
    op.execute(
        'UPDATE events SET start = (date + time_start)::timestamp, "end" = (date + time_end)::timestamp'
    )
